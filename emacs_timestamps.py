...
BITS = 64
EMACS_LSBITS = 16
MICRO_MULTIPLIER = 1e-6
PICO_MULTIPLIER = 1e-12

LSB_MASK = (1 << EMACS_LSBITS) - 1
MSB_MASK = ~LSB_MASK & ((1 << BITS) -1) # Strip off two's complement

print( f"LSB_MASK = {LSB_MASK:064b}")
print( f"MSB_MASK = {MSB_MASK:064b}")
...
# Out[93]: 1710215080
x = (((ms_bits << EMACS_LSBITS ) & MSB_MASK) | (ls_bits & LSB_MASK) ) + (µ * MICRO_MULTIPLIER) + (pico * PICO_MULTIPLIER)
print( x)
